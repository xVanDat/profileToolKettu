() => {
    ...
    const nitroBadges = [
        ["Nitro", "2ba85e8026a8614b640c2837bcdfe21b", 0],
        ["Bronze", "4f33c4a9c64ce221936bd256c356f91f", 1],
        ["Silver", "4514fab914bdbfb4ad2fa23df76121a6", 3],
        ["Gold", "2895086c18d5531d499862e41d1155a6", 6],
        ["Platinum", "0334688279c8359120922938dcb1d6f8", 12],
        ["Diamond", "0d61871f72bb9a33a7ae568c1fb4f20a", 24],
        ["Emerald", "11e2d339068b55d3a506cff34d3780f3", 36],
        ["Ruby", "cd5e2cfd9d7f27a8cdcd3e8a8d5dc9f4", 60],
        ["Opal", "5b154df19c53dce2af92c9b61e6be5e2", 72],
    ];
    const boostBadges = [
        ["1 Month Server Booster", "51040c70d4f20a921ad6674ff86fc95c", 1],
        ["2 Month Server Booster", "0e4080d1d333bc7ad29ef6528b6f2fb7", 2],
        ["3 Month Server Booster", "72bed924410c304dbe3d00a6e593ff59", 3],
        ["6 Month Server Booster", "df199d2050d3ed4ebf84d64ae83989f8", 6],
        ["9 Month Server Booster", "996b3e870e8a22ce519b3a50e6bdd52f", 9],
        ["12 Month Server Booster", "991c9f39ee33d7537d9f408c3e53141e", 12],
        ["15 Month Server Booster", "cb3ae83c15e970e8f3d410bc62cb8b99", 15],
        ["18 Month Server Booster", "7142225d31238f6387d9f09efaa02759", 18],
        ["24 Month Server Booster", "ec92202290b48d0879b7413d2dde3bab", 24],
    ];
    ...
    const visualKeys = [
        ...
        "avatarDecorationData", "profileEffectId", "profileEffect", "premiumSince",
        "premiumGuildSince", "themeColors",
    ];
    ...
    function customValues(userId) {
        ...
        if (custom.nitro) {
            const nitroLevel = Math.max(0, Math.min(nitroBadges.length - 1, Number(custom.nitroLevel) || 0));
            const nitroDate = new Date();
            ...
            if (custom.boostLevel !== undefined && Number(custom.boostLevel) >= 0) {
                const boostLevel = Math.max(0, Math.min(boostBadges.length - 1, Number(custom.boostLevel) || 0));
    ...
    function profileBadges(userId) {
        ...
        const premiumBadges = [];
        if (profile.nitro) {
            const nitroLevel = Math.max(0, Math.min(nitroBadges.length - 1, Number(profile.nitroLevel) || 0));
            const nitro = nitroBadges[nitroLevel];
            premiumBadges.push({
            ...
            if (profile.boostLevel !== undefined && Number(profile.boostLevel) >= 0) {
                const boostLevel = Math.max(0, Math.min(boostBadges.length - 1, Number(profile.boostLevel) || 0));
                const boost = boostBadges[boostLevel];
                premiumBadges.push({
        ...
        return h(ScrollView, { style: styles.screen, contentContainerStyle: styles.content },
            ...
            h(View, { style: styles.section },
                ...
                h(Toggle, {
                    ...
                    onValueChange: value => setDraft(previous => ({ ...previous, nitro: value })),
                }),
                draft.nitro ? h(Field, {
                ...
                draft.nitro ? h(Field, {
                    ...
                    onChangeText: value => setDraft(previous => ({
                        ...previous,
                        boostLevel: value.trim() === "-1" || !value.trim()
                            ? -1
                            : Math.max(0, Math.min(8, Number.parseInt(value, 10) || 0)),
                    })),
                }) : null,
                draft.nitro ? h(Text, { style: styles.hint }, "Nitro level: 0 Nitro, 1 Bronze, 2 Silver, 3 Gold, 4 Platinum, 5 Diamond, 6 Emerald, 7 Ruby, 8 Opal. Booster uses levels 0–8 for 1–24 months.") : null,
                h(Field, {
